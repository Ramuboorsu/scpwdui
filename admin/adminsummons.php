...
if(isset($_POST['submit'])){
    ...
    $desc = $_POST['desc'];
     ...
     // $RandomNum   = time();
                ...
                $insert_img = mysqli_query($con,"insert into summons(name,description,doc) values('$name','$desc','$NewImageName')");
                  // $result = mysqli_array($insert_img);
                if($insert_img)
                {
                  echo '<script>alert("Successfully Inserted");window.location.href="adminsummons.php";</script>';
                }
    } 
    if(@$_GET['step'] == 'delete')
    {
    $id = @$_GET['q'];
        $del = mysqli_query($con,"delete from summons where id = $id");
        if($del)
        {
            echo '<script>alert("Successfully Deleted");window.location.href="adminsummons.php";</script>';
                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                            <table id="zero_config" class="table table-striped table-bordered no-wrap">
                                ...
                                <tbody>
                                <?php
$sel = mysqli_query($con,"select * from summons");
$cnt = 1;
...
?>
            <tr>
                ...
                <td><a href="../images/summons/<?php echo $doc; ?>" download><?php echo $doc; ?></a></td>
                <td>
                 
                  <a href="adminsummons.php?step=delete&q=<?php echo $id; ?>" onClick="return deleteconfirm()" class="btn btn-danger"><i class='far'>&#xf2ed;</i></a>
              &nbsp;&nbsp;
                 <a href="adminfilesupdate.php?step=summonupdate&q=<?php echo $id; ?>"  class="btn" id="updatebtn"><i class='far'>&#xf044;</i></a>
